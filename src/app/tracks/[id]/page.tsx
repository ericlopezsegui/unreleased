...
export default function TrackPage() {
  ...
  const submitVersion = async () => {
    if (!vLabel.trim()) {
      setVError('El nombre de la versión es obligatorio')
      return
    }
    ...
    if (vAudioFile && track) {
      ...
      if (upErr) {
        setVError(`Error al subir audio: ${upErr.message}`)
    ...
    const { data: newVer, error: dbErr } = await supabase
      .from('track_versions')
      .insert({
        track_id: id,
        label: vLabel.trim(),
      ...
      .select('id,label,notes,audio_path,bpm,key,is_active,created_at')
      .single()

    if (dbErr) {
      setVError(dbErr.message)
      setVSubmitting(false)
      return
    }
      ...
      <div style={{ maxWidth: 560, margin: '0 auto', padding: '24px 20px 140px', animation: 'fadeUp .38s ease both' }}>
        ...
        {activeVersion && (activeVersion.bpm || activeVersion.key) && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 20 }}>
            ...
            <span style={{ fontSize: 11, color: '#999', background: 'rgba(0,0,0,0.03)', padding: '4px 10px', borderRadius: 20, fontWeight: 400 }}>
        ...
        <div onClick={closeAddVersion} className="edit-overlay" style={{ background: addVerVisible ? 'rgba(0,0,0,0.25)' : 'rgba(0,0,0,0)', backdropFilter: addVerVisible ? 'blur(8px)' : 'blur(0px)' }}>
          <div onClick={e => e.stopPropagation()} className="edit-panel" style={{ transform: addVerVisible ? 'translateY(0)' : 'translateY(100%)', opacity: addVerVisible ? 1 : 0 }}>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <div
                onDrop={e => { e.preventDefault(); const f = e.dataTransfer.files[0]; if (f?.type.startsWith('audio/') && !vAnalyzing) void handleVAudio(f) }}
              ...
              <input
                ref={vAudioRef}
                type="file"
                accept="audio/*,.mp3,.wav,.aac,.ogg,.flac,.m4a,.mp4,audio/mpeg,audio/mp3,audio/wav,audio/aac,audio/ogg,audio/flac,audio/x-m4a,audio/mp4"
                onChange={e => {
                  const f = e.target.files?.[0]
                  if (f && !vAnalyzing) void handleVAudio(f)
                }}
                style={{ display: 'none' }}
              />
            </div>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  ...
                  {editCoverFile && (
                    <button type="button" onClick={() => { setEditCoverFile(null); setEditCoverPreview(track?.cover_path ? coverUrl : null) }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#ccc', fontSize: 11, fontFamily: 'inherit', textAlign: 'left', padding: 0 }}>
                      Descartar cambio
                    </button>
                  )}
