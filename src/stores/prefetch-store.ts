...
interface PrefetchState {
  ...
  stems: PrefetchStem[]
  stemAudioUrls: Record<string, string>
  setStemAudioUrl: (stemId: string, url: string) => void
...
export const usePrefetchStore = create<PrefetchState>((set, get) => ({
  ...
  stems: [],
  stemAudioUrls: {},
  setStemAudioUrl: (stemId, url) => set(s => ({ stemAudioUrls: { ...s.stemAudioUrls, [stemId]: url } })),
  ...
  prefetch: async () => {
    ...
    const [profileRes, membershipRes] = await Promise.all([
      supabase
        .from('profiles')
        .select('display_name,avatar_path,theme,onboarding_completed')
        .eq('user_id', user.id)
        .single(),
      supabase
        .from('artist_members')
    ...
    const [artistRes, albumsRes, tracksRes, membersRes] = await Promise.all([
      supabase
        .from('artists')
        .select('id,name,handle,avatar_path,bio,owner_user_id')
        .eq('id', artistId)
        .single(),
      supabase
        .from('albums')
      ...
      supabase
        .from('tracks')
        .select('id,title,description,cover_path,album_id,position,updated_at,artist_id,albums(title,cover_path)')
        .eq('artist_id', artistId)
        .order('updated_at', { ascending: false }),
    ...
    trackList.forEach(t => {
      if (t.album_id) countMap[t.album_id] = (countMap[t.album_id] ?? 0) + 1
    })
    albumList.forEach(a => {
      a.track_count = countMap[a.id] ?? 0
    })
    ...
    /* 4. Load stems for all versions */
    const versionIds = allVersions.map(v => v.id)
    let stems: PrefetchStem[] = []

    if (versionIds.length > 0) {
      const { data: stemsData } = await supabase
        .from('stems')
        .select('id,version_id,stem_type,label,audio_path')
        .in('version_id', versionIds)

      stems = (stemsData ?? []) as PrefetchStem[]
    }

    /* 5. Signed URLs — covers, audio, avatars */
    const coverUrls: Record<string, string> = {}
    const audioUrls: Record<string, string> = {}
    const stemAudioUrls: Record<string, string> = {}
    ...
    type UrlReq = {
      ...
      path: string
      ttl: number
      target: 'cover' | 'audio' | 'stem' | 'avatar' | 'artistAvatar' | 'memberAvatar'
    }
    ...
    memberList.forEach(m => {
      if (m.profile?.avatar_path) {
        reqs.push({
          key: m.user_id,
          bucket: 'avatars',
          path: m.profile.avatar_path,
          ttl: 86400,
          target: 'memberAvatar',
        })
      }
    ...
    for (let i = 0; i < reqs.length; i += BATCH) {
      ...
      const results = await Promise.all(
        batch.map(r =>
          supabase.storage.from(r.bucket).createSignedUrl(r.path, r.ttl)
        )
      )
